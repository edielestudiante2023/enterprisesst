...
echo "Creando tablas de indicadores SST...\n";
...
// Tabla de histórico de mediciones
$sql2 = "
...
try {
    $pdo->exec($sql1);
    echo "Tabla tbl_indicadores_sst creada exitosamente!\n";

    $pdo->exec($sql2);
    echo "Tabla tbl_indicadores_sst_mediciones creada exitosamente!\n";
...
// Verificar tablas
echo "\n=== Verificando tabla tbl_indicadores_sst ===\n";
$stmt = $pdo->query("DESCRIBE tbl_indicadores_sst");
while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    echo "- " . $row['Field'] . " (" . $row['Type'] . ")\n";
}

echo "\n=== Verificando tabla tbl_indicadores_sst_mediciones ===\n";
$stmt = $pdo->query("DESCRIBE tbl_indicadores_sst_mediciones");
while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    echo "- " . $row['Field'] . " (" . $row['Type'] . ")\n";
}
