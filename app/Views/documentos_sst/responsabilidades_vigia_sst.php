...
<body class="bg-light">
        ...
        <div class="container-fluid">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    ...
                    <span class="ms-3"><?= esc($cliente['nombre_cliente']) ?> - Responsabilidades Vigia SST <?= $anio ?></span>
                </div>
                <div>
    ...
    <div class="container my-4">
        <div class="bg-white shadow documento-contenido" style="padding: 40px; max-width: 900px; margin: 0 auto;">
                ...
                <div class="row align-items-center">
                    <div class="col-md-8">
                        ...
                        <small class="opacity-75">
                            ...
                            Documento exclusivo para empresas con 7 estandares
                        </small>
                    </div>
                </div>
            </div>
            ...
            <div class="info-documento no-print">
                <div class="row align-items-center">
                    <div class="col-md-8">
                        ...
                        <strong>7 Estandares:</strong> Este documento aplica solo para empresas con menos de 10 trabajadores (Riesgo I, II o III).
                    </div>
                    <div class="col-md-4 text-end">
                        <span class="badge bg-success">Firma Digital: Vigia SST</span>
                    </div>
                </div>
            </div>
            ...
            <table class="encabezado-formal">
                    ...
                    <td class="encabezado-info" rowspan="2">
                        <table class="encabezado-info-table">
                            <tr>
                                ...
                                <td><?= esc($documento['codigo'] ?? 'RES-VIG-001') ?></td>
                            </tr>
                            <tr>
                ...
                <tr>
                    <td class="encabezado-titulo-central">
                        <div class="nombre-doc">RESPONSABILIDADES DEL VIGIA DE SEGURIDAD Y SALUD EN EL TRABAJO</div>
                    </td>
                </tr>
            </table>
            ...
            <!-- Seccion de Firma -->
            <?php
            $vigiaData = $contenido['vigia'] ?? [];
            $firmaVigia = ($firmasElectronicas ?? [])['vigia_sst'] ?? null;
            ?>
                        ...
                        <tr>
                            <td style="vertical-align: top; padding: 25px; height: 200px; position: relative;">
                                <div class="text-center mb-3">
                                    <strong style="color: #495057; font-size: 1rem;">VIGIA DE SEGURIDAD Y SALUD EN EL TRABAJO</strong>
                                </div>
                                ...
                                <div style="margin-bottom: 10px;">
                                    ...
                                    <span style="border-bottom: 1px dotted #999; display: inline-block; min-width: 200px; padding-bottom: 2px;">
                                        <?= esc($vigiaData['cedula'] ?? '') ?>
                                    </span>
                                </div>
                                <div style="margin-bottom: 10px;">
                                    <strong style="color: #495057;">Cargo:</strong>
                                    <span><?= esc($vigiaData['cargo'] ?? 'Vigia de SST') ?></span>
                                </div>
                                <div style="text-align: center; margin-top: 30px;">
                                    <?php if ($firmaVigia && !empty($firmaVigia['evidencia']['firma_imagen'])): ?>
                                        <img src="<?= $firmaVigia['evidencia']['firma_imagen'] ?>" alt="Firma Vigia" style="max-height: 70px; max-width: 200px; margin-bottom: 5px;">
                                    <?php endif; ?>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="alert alert-info">
                        ...
                        Se actualizara el documento con los datos actuales del Vigia SST registrado en el modulo de Responsables.
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Vigia SST actual:</label>
                        <div class="alert alert-light border">
                            <?php if ($vigia): ?>
                                <strong><?= esc($vigia['nombre_completo'] ?? 'No definido') ?></strong><br>
                                <small class="text-muted">Documento: <?= esc($vigia['numero_documento'] ?? '') ?></small>
                            <?php else: ?>
                                <span class="text-danger">No hay Vigia SST registrado</span>
                            <?php endif; ?>
                        </div>
                    </div>
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="descripcionCambio" rows="2"
                                  placeholder="Ej: Cambio de Vigia SST"></textarea>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
                    <button type="button" class="btn btn-warning" id="btnConfirmarRegenerar" <?= !$vigia ? 'disabled' : '' ?>>
                        <i class="bi bi-arrow-repeat me-1"></i>Actualizar
                    </button>
                </div>
            </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('btnConfirmarRegenerar')?.addEventListener('click', function() {
            ...
            fetch('<?= base_url("documentos-sst/{$cliente['id_cliente']}/regenerar-responsabilidades-vigia-sst/{$anio}") ?>', {
                method: 'POST',
                body: formData
            })
            .then(response => response.json())
