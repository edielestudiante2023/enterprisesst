...
<head>
    ...
    <style>
        ...
        .documento-cuerpo { text-align: justify; line-height: 1.8; font-size: 0.95rem; margin-bottom: 30px; }
        .panel-aprobacion { background: linear-gradient(135deg, #f59e0b 0%, #b45309 100%); border-radius: 12px; padding: 20px; margin-bottom: 20px; color: white; }
        .panel-aprobacion .estado-badge { font-size: 1rem; padding: 8px 16px; }
        .info-documento { background-color: #f8f9fa; padding: 15px; border-radius: 8px; margin-bottom: 20px; }
    </style>
</head>
<body class="bg-light">
        ...
        <div class="container-fluid">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    ...
                    <span class="ms-3"><?= esc($cliente['nombre_cliente']) ?> - Certificacion No Sustancias Cancerigenas <?= $anio ?></span>
                </div>
                <div>
    ...
    <div class="container my-4">
        <div class="bg-white shadow documento-contenido" style="padding: 40px; max-width: 900px; margin: 0 auto;">
            ...
            <table class="encabezado-formal">
                    ...
                    <td class="encabezado-info" rowspan="2">
                        <table class="encabezado-info-table">
                            <tr>
                                ...
                                <td><?= esc($documento['codigo'] ?? 'CERT-NSC-001') ?></td>
                            </tr>
                            <tr>
                ...
                <tr>
                    <td class="encabezado-titulo-central">
                        <div class="nombre-doc">CERTIFICACION DE NO SUSTANCIAS CANCERIGENAS / TOXICIDAD AGUDA</div>
                    </td>
                </tr>
            </table>
                    ...
                    <?php if (!empty($versiones)): ?>
                        ...
                        <tr>
                            <td style="text-align: center;">
                                <span style="display: inline-block; background: #0d6efd; color: white; padding: 3px 12px; border-radius: 20px; font-weight: bold;"><?= esc($ver['version_texto']) ?></span>
                            </td>
            ...
            <!-- Firmas (patron 3 columnas: Consultor + Delegado SST + Representante Legal) -->
            <div class="firma-section" style="margin-top: 40px; page-break-inside: avoid;">
                ...
                <table class="table table-bordered mb-0" style="font-size: 0.85rem; border-top: none;">
                    <thead>
                        <tr style="background: linear-gradient(135deg, #f8f9fa, #e9ecef);">
                            <th style="width: 33%; text-align: center; font-weight: 600; color: #495057; border-top: none;">
                                <i class="bi bi-pencil-square me-1"></i>ELABORO
                            </th>
                            <th style="width: 33%; text-align: center; font-weight: 600; color: #495057; border-top: none;">
                                <i class="bi bi-shield-check me-1"></i>REVISO / DELEGADO SST
                            </th>
                            <th style="width: 34%; text-align: center; font-weight: 600; color: #495057; border-top: none;">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td style="vertical-align: top; padding: 15px; height: 200px; position: relative;">
                                <div style="margin-bottom: 6px;">
                                    <strong style="color: #495057; font-size: 0.8rem;">Nombre:</strong>
                                    <span style="font-size: 0.85rem;"><?= !empty($consultorNombre) ? esc($consultorNombre) : '' ?></span>
                                </div>
                                <div style="margin-bottom: 6px;">
                                ...
                                <div style="position: absolute; bottom: 12px; left: 15px; right: 15px; text-align: center;">
                                    <?php if ($firmaConsultorElectronica && !empty($firmaConsultorElectronica['evidencia']['firma_imagen'])): ?>
                                        <img src="<?= $firmaConsultorElectronica['evidencia']['firma_imagen'] ?>" alt="Firma Consultor" style="max-height: 60px; max-width: 150px; margin-bottom: 3px;">
                                    <?php elseif (!empty($firmaConsultorFisica)): ?>
                                        <img src="<?= base_url('uploads/' . $firmaConsultorFisica) ?>" alt="Firma Consultor" style="max-height: 60px; max-width: 150px; margin-bottom: 3px;">
                                    <?php endif; ?>
                                    <div style="border-top: 1px solid #333; width: 85%; margin: 0 auto; padding-top: 4px;"><small style="color: #666; font-size: 0.7rem;">Firma</small></div>
                                </div>
                            </td>
                            ...
                            <td style="vertical-align: top; padding: 15px; height: 200px; position: relative;">
                                <div style="margin-bottom: 6px;">
                                    <strong style="color: #495057; font-size: 0.8rem;">Nombre:</strong>
                                    <span style="font-size: 0.85rem;"><?= !empty($delegadoNombre) ? esc($delegadoNombre) : '' ?></span>
                                </div>
                                <div style="margin-bottom: 6px;">
                                    <strong style="color: #495057; font-size: 0.8rem;">Cargo:</strong>
                                    <span style="font-size: 0.85rem;"><?= esc($delegadoCargo ?? 'Lider de Operaciones') ?></span>
                                </div>
                                <div style="position: absolute; bottom: 12px; left: 15px; right: 15px; text-align: center;">
                                    <?php $firmaDelegado = ($firmasElectronicas ?? [])['delegado_sst'] ?? null; if ($firmaDelegado && !empty($firmaDelegado['evidencia']['firma_imagen'])): ?>
                                        <img src="<?= $firmaDelegado['evidencia']['firma_imagen'] ?>" alt="Firma Delegado SST" style="max-height: 60px; max-width: 150px; margin-bottom: 3px;">
                                    <?php endif; ?>
                                    <div style="border-top: 1px solid #333; width: 85%; margin: 0 auto; padding-top: 4px;"><small style="color: #666; font-size: 0.7rem;">Firma</small></div>
                                </div>
                            </td>
                            ...
                            <td style="vertical-align: top; padding: 15px; height: 200px; position: relative;">
                                <div style="margin-bottom: 6px;">
                                    <strong style="color: #495057; font-size: 0.8rem;">Nombre:</strong>
                                    <span style="font-size: 0.85rem;"><?= !empty($repLegalNombre) ? esc($repLegalNombre) : '' ?></span>
                                </div>
                                <div style="margin-bottom: 6px;">
                                    <strong style="color: #495057; font-size: 0.8rem;">Cargo:</strong>
                                ...
                                <div style="margin-bottom: 6px;">
                                    <strong style="color: #495057; font-size: 0.8rem;">C.C.:</strong>
                                    <span style="font-size: 0.85rem;"><?= esc($repLegalCedula) ?></span>
                                </div>
                                <?php endif; ?>
                                <div style="position: absolute; bottom: 12px; left: 15px; right: 15px; text-align: center;">
                                    <?php $firmaRepLegal = ($firmasElectronicas ?? [])['representante_legal'] ?? null; if ($firmaRepLegal && !empty($firmaRepLegal['evidencia']['firma_imagen'])): ?>
                                        <img src="<?= $firmaRepLegal['evidencia']['firma_imagen'] ?>" alt="Firma Rep. Legal" style="max-height: 60px; max-width: 150px; margin-bottom: 3px;">
                                    <?php endif; ?>
                                    <div style="border-top: 1px solid #333; width: 85%; margin: 0 auto; padding-top: 4px;"><small style="color: #666; font-size: 0.7rem;">Firma</small></div>
                                </div>
                            </td>
    ...
    <?= view('documentos_sst/_components/modal_historial_versiones', ['id_documento' => $documento['id_documento'], 'versiones' => $versiones ?? []]) ?>
    <?= view('documentos_sst/_components/modal_nueva_version', ['id_documento' => $documento['id_documento'], 'version_actual' => ($documento['version'] ?? 1) . '.0', 'tipo_documento' => $documento['tipo_documento'] ?? 'certificacion_no_sustancias_cancerigenas', 'es_primera_aprobacion' => empty($versiones ?? [])]) ?>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                        ...
                        <div class="card-body">
                            <div class="row g-3">
                                <div class="col-md-8">
                                    ...
                                    <input type="text" class="form-control" id="regenerarRepLegalNombre" value="<?= esc($contexto['representante_legal_nombre'] ?? '') ?>">
                                </div>
                                <div class="col-md-4">
                                    <label for="regenerarRepLegalCedula" class="form-label">Cedula</label>
                                    <input type="text" class="form-control" id="regenerarRepLegalCedula" value="<?= esc($contexto['representante_legal_cedula'] ?? '') ?>">
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="descripcionCambio" rows="2" placeholder="Ej: Cambio de representante legal"></textarea>
                    </div>
                </div>
    ...
    function mostrarToast(tipo, titulo, mensaje) {
        ...
        const html = `<div id="${toastId}" class="toast" role="alert" style="min-width:300px;box-shadow:0 4px 12px rgba(0,0,0,.15);margin-bottom:8px;">
        ...
        const inst = new bootstrap.Toast(el, {delay: duracion});
    ...
    document.addEventListener('DOMContentLoaded', function() {
        document.getElementById('btnConfirmarRegenerar')?.addEventListener('click', function() {
            const btn = this;
            const descripcion = document.getElementById('descripcionCambio').value.trim() || 'Actualizacion de datos de representante legal';
            ...
            fetch('<?= base_url("documentos-sst/{$cliente['id_cliente']}/regenerar-certificacion-sustancias-cancerigenas/{$anio}") ?>', {
                method: 'POST',
                body: formData
            })
            .then(r => r.json())
