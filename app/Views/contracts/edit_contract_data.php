...
<body>
    ...
    <div class="container">
        ...
        <form action="<?= base_url('/contracts/save-and-generate/' . $contract['id_contrato']) ?>" method="post">
            ...
            <!-- Sección 6: Cláusula Primera - Objeto del Contrato -->
            <div class="form-section">
                <h4 class="section-title"><i class="fas fa-bullseye"></i> Cláusula Primera - Objeto del Contrato</h4>

                <div class="alert alert-info">
                    <i class="fas fa-info-circle"></i>
                    <strong>Cláusula Personalizable:</strong> Defina el objeto del contrato. Si el consultor es externo,
                    se incluirá automáticamente una cláusula de delegación de visitas presenciales.
                </div>
                ...
                <!-- Barra de herramientas post-generación Cláusula 1 -->
                <div class="gap-2 mb-2" id="toolbarIAClausula1" style="display: <?= !empty($contract['clausula_primera_objeto']) ? 'flex' : 'none' ?>;">
                ...
                <div class="mb-3">
                    ...
                    <textarea name="clausula_primera_objeto" id="clausula_primera_objeto" class="form-control" rows="12"
                              placeholder="Ejemplo:&#10;&#10;PRIMERA-OBJETO DEL CONTRATO: EL CONTRATISTA se compromete a proporcionar servicios de consultoría para el diseño e implementación del Sistema de Gestión de Seguridad y Salud en el Trabajo (SG-SST) a favor de EL CONTRATANTE mediante la plataforma EnterpriseSST...&#10;&#10;Se asignará al profesional en SG-SST NOMBRE COMPLETO, identificado con cédula de ciudadanía No. XXXXX y licencia SST No. XXXXX, como responsable técnico y coordinador del servicio..."><?= esc($contract['clausula_primera_objeto'] ?? '') ?></textarea>
                    <small class="text-muted">
                        Este texto aparecerá en el PDF del contrato como la CLÁUSULA PRIMERA (Objeto). Personalícelo según las condiciones del contrato.
                    </small>
                </div>
            </div>
    ...
    <script>
        ...
        function abrirSweetAlertIAClausula1(precargar = false) {
            const nombreCoord = document.getElementById('nombre_responsable_sgsst').value;
            const cedulaCoord = document.getElementById('cedula_responsable_sgsst').value;
            const licenciaCoord = document.getElementById('licencia_responsable_sgsst').value;
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                ...
                                <input type="text" id="swal_c1_nombre" class="form-control"
                                       value="${precargar ? (ultimosDatosClausula1.nombre_coordinador || nombreCoord) : nombreCoord}">
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">Cédula</label>
                                <input type="text" id="swal_c1_cedula" class="form-control"
                                       value="${precargar ? (ultimosDatosClausula1.cedula_coordinador || cedulaCoord) : cedulaCoord}">
                            </div>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Licencia SST</label>
                            <input type="text" id="swal_c1_licencia" class="form-control"
                                   value="${precargar ? (ultimosDatosClausula1.licencia_coordinador || licenciaCoord) : licenciaCoord}">
                        </div>
                        <div class="mb-3">
                            <label class="form-label fw-bold">Contexto adicional <small class="text-muted">(opcional)</small></label>
        ...
        function abrirRefinarClausula1() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    const payload = {
                        id_cliente: '<?= $contract['id_cliente'] ?>',
                        nombre_coordinador: document.getElementById('nombre_responsable_sgsst').value,
                        cedula_coordinador: document.getElementById('cedula_responsable_sgsst').value,
                        licencia_coordinador: document.getElementById('licencia_responsable_sgsst').value,
                        contexto_adicional: result.value,
                        texto_actual: textoActual,
                        modo_refinamiento: true
                    };
