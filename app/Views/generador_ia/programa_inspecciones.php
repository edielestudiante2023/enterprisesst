...
<head>
    ...
    <title>Generador Programa Inspecciones - <?= esc($cliente['nombre_cliente']) ?></title>
...
<body class="bg-light">
    <nav class="navbar navbar-expand-lg navbar-dark bg-primary">
        <div class="container-fluid">
            <a class="navbar-brand" href="#">
                <i class="bi bi-search me-2"></i>Generador IA - Programa de Inspecciones
            </a>
    ...
    <div class="container-fluid py-4">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h4 class="mb-1">Programa de Inspecciones a Instalaciones, Maquinaria o Equipos</h4>
                <p class="text-muted mb-0">Estandar 4.2.4 - Resolucion 0312/2019 - Con participacion del COPASST</p>
            </div>
            <div>
        ...
        <div class="alert alert-info mb-4">
            ...
            <strong>Flujo de generacion del Programa de Inspecciones:</strong>
            <ol class="mb-0 mt-2">
                <li><strong>Actividades de Inspecciones</strong> - Se generan actividades de inspeccion en el PTA (instalaciones, maquinaria, EPP, emergencia)</li>
                <li><strong>Indicadores de Inspecciones</strong> - Se configuran indicadores para medir el programa</li>
        ...
        <div class="card border-0 shadow-sm mb-4">
            <div class="card-header bg-white d-flex justify-content-between align-items-center">
                <h6 class="mb-0">
                    ...
                    <small class="text-muted ms-2">La IA usara esta informacion para generar actividades de inspecciones personalizadas</small>
                </h6>
                    ...
                    <div class="row">
                        <div class="col-12">
                            ...
                            <textarea id="instruccionesIA" class="form-control" rows="3"
                                placeholder="Ej: Incluir inspecciones de trabajo en alturas, agregar inspeccion de vehiculos, enfocarse en areas de almacenamiento quimico..."></textarea>
                            <small class="text-muted">
                                Describa necesidades especificas para personalizar las actividades de inspecciones.
                            </small>
                        </div>
                    </div>
                </div>
            ...
            <div class="card-body py-3">
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <div class="d-flex align-items-center gap-3">
                        <span>Actividades existentes: <span class="badge bg-<?= ($resumenActividades['existentes'] ?? 0) > 0 ? 'success' : 'warning' ?>"><?= $resumenActividades['existentes'] ?? 0 ?> actividades</span></span>
        ...
        <div class="card border-0 shadow-sm mb-4">
            <div class="card-header bg-white">
                <h6 class="mb-0"><i class="bi bi-list-check text-success me-2"></i>Actividades de Inspecciones en el PTA (<?= count($actividadesExistentes) ?>)</h6>
            </div>
                ...
                <div class="table-responsive">
                    <table class="table table-hover mb-0">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>
                                    <strong><?= esc($act['actividad_plandetrabajo']) ?></strong>
                                    <?php if (!empty($act['responsable_sugerido_plandetrabajo'])): ?>
                                        <br><small class="text-muted"><?= esc($act['responsable_sugerido_plandetrabajo']) ?></small>
                                    <?php endif; ?>
                                </td>
                                <td><?= esc($act['fecha_propuesta'] ?? 'N/A') ?></td>
                                <td><span class="badge bg-info"><?= esc(strtoupper($act['phva_plandetrabajo'] ?? 'H')) ?></span></td>
                                <td>
                                    <?php $estado = $act['estado_actividad'] ?? 'ABIERTA'; $colorEstado = match($estado) { 'CERRADA' => 'success', 'EN_PROGRESO' => 'warning', default => 'secondary' }; ?>
        ...
        <div class="card border-0 shadow-sm mb-4">
            <div class="card-header bg-primary bg-opacity-25">
                <h6 class="mb-0"><i class="bi bi-magic me-2"></i>Generar Actividades de Inspecciones</h6>
            </div>
            <div class="card-body">
                <p class="text-muted">Se generaran actividades de inspeccion a instalaciones, maquinaria y equipos adaptadas al contexto de la empresa.</p>
                <div class="d-flex gap-2">
                    ...
                    <button type="button" class="btn btn-primary" id="btnGenerar" onclick="generarActividades()" disabled>
                        <i class="bi bi-lightning me-1"></i>Generar en PTA
                    </button>
                    <a href="<?= base_url('generador-ia/' . $cliente['id_cliente'] . '/indicadores-programa-inspecciones') ?>" class="btn btn-outline-success ms-auto">
    ...
    async function previewActividades() {
        ...
        const instrucciones = document.getElementById('instruccionesIA')?.value || '';

        try {
            const response = await fetch(`<?= base_url('generador-ia') ?>/${idCliente}/preview-actividades-inspecciones?anio=${anio}&instrucciones=${encodeURIComponent(instrucciones)}`);
            ...
            if (data.success) {
                ...
                if (data.data.explicacion_ia) {
                    showAlert('info', '<i class="bi bi-robot me-1"></i>' + data.data.explicacion_ia);
                }
            } else {
    ...
    function renderPreview(actividades) {
        ...
        actividades.forEach((act, idx) => {
            const origen = act.origen === 'ia' ? '<span class="badge bg-purple text-white" style="background:#6f42c1">IA</span>' : '';
            tbody.innerHTML += `
                <tr>
                    <td><input type="checkbox" class="actCheck" data-idx="${idx}" checked></td>
                    <td><strong>${act.actividad}</strong> ${origen}</td>
                    <td><small class="text-muted">${(act.objetivo || '').substring(0, 100)}</small></td>
                    <td>${act.mes || 'N/A'}</td>
                    <td><span class="badge bg-${phvaColors[act.phva] || 'info'}">${(act.phva || 'HACER').toUpperCase()}</span></td>
                    <td><small>${act.responsable || 'Responsable SST'}</small></td>
                </tr>
            `;
        });
    }
    ...
    async function generarActividades() {
        ...
        try {
            const response = await fetch(`<?= base_url('generador-ia') ?>/${idCliente}/generar-actividades-inspecciones`, {
                method: 'POST',
