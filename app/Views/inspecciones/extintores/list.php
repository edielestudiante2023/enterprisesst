<div class="container-fluid px-3">
    ...
    <!-- Boton nueva -->
    <a href="<?= site_url('inspecciones/extintores/create') ?>" class="btn btn-pwa btn-pwa-primary mb-3">
    ...
    <?php if (empty($inspecciones)): ?>
        <div class="text-center text-muted py-4">
            <i class="fas fa-fire-extinguisher" style="font-size:40px; opacity:0.3;"></i>
            <p class="mt-2">No hay inspecciones de extintores</p>
        </div>
    <?php else: ?>
        ...
        <div class="card card-inspeccion mb-2 insp-card" data-cliente="<?= esc($insp['nombre_cliente'] ?? '') ?>">
            <div class="card-body py-2 px-3">
                <div class="d-flex justify-content-between align-items-start">
                    <div style="flex:1;">
                        ...
                        <div class="text-muted" style="font-size:12px;">
                            ...
                            &middot; <?= $insp['total_detalles'] ?? 0 ?> extintor(es)
                            <?php if (!empty($insp['numero_extintores_totales'])): ?>
                                &middot; Total: <?= $insp['numero_extintores_totales'] ?>
                            <?php endif; ?>
                        </div>
                    </div>
                ...
                <div class="mt-2 d-flex gap-1 flex-wrap">
                    <!-- Doc 25: edit/delete buttons always visible regardless of estado -->
                    <a href="<?= site_url('inspecciones/extintores/edit/' . $insp['id']) ?>" class="btn btn-sm btn-outline-dark">
                    ...
                    <?php if ($insp['estado'] === 'completo'): ?>
                        <a href="<?= site_url('inspecciones/extintores/view/' . $insp['id']) ?>" class="btn btn-sm btn-outline-dark">
                            <i class="fas fa-eye"></i> Ver
                        </a>
                        <a href="<?= site_url('inspecciones/extintores/pdf/' . $insp['id']) ?>" class="btn btn-sm btn-outline-primary" target="_blank">
    ...
    document.addEventListener('click', function(e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                window.location.href = '<?= site_url('inspecciones/extintores/delete/') ?>' + btn.dataset.id;
            }
        });
    });
});
