...
<div class="container-fluid px-3">
    ...
    <div class="d-flex justify-content-between align-items-center mt-2 mb-3">
        <h6 class="mb-0">Inspecciones de Extintores</h6>
        <a href="<?= site_url('inspecciones/extintores/create') ?>" class="btn btn-sm btn-pwa-primary" style="width:auto; padding: 8px 16px;">
        ...
        <table id="tablaInspecciones" class="table table-striped table-bordered table-hover" style="width:100%">
            ...
            <?php foreach (($inspecciones ?? []) as $insp): ?>
                <tr>
                    ...
                    <td><?= (int)($insp['total_detalles'] ?? 0) ?></td>
                    <td><?= (int)($insp['numero_extintores_totales'] ?? 0) ?></td>
                    <td><span class="badge badge-<?= esc($insp['estado']) ?>"><?= $insp['estado'] === 'completo' ? 'Completo' : 'Borrador' ?></span></td>
                    <td class="table-actions">
                        <a href="<?= site_url('inspecciones/extintores/edit/' . $insp['id']) ?>" class="btn btn-outline-dark" title="Editar"><i class="fas fa-edit"></i></a>
                        <?php if ($insp['estado'] === 'completo'): ?>
                            <a href="<?= site_url('inspecciones/extintores/view/' . $insp['id']) ?>" class="btn btn-outline-dark" title="Ver"><i class="fas fa-eye"></i></a>
                            <a href="<?= site_url('inspecciones/extintores/pdf/' . $insp['id']) ?>" class="btn btn-outline-success" target="_blank" title="PDF"><i class="fas fa-file-pdf"></i></a>
                        <?php endif; ?>
                        <a href="<?= site_url('inspecciones/extintores/delete/' . $insp['id']) ?>" class="btn btn-outline-danger btn-delete" title="Eliminar"><i class="fas fa-trash"></i></a>
                    </td>
                </tr>
            <?php endforeach; ?>
            </tbody>
