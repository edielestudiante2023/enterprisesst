...
<head>
    <meta charset="UTF-8">
    <title>Inscripcion - <?= esc($lista['motivo'] ?? 'Convocatoria') ?></title>
...
<body>
    <div class="card card-inscripcion">
        ...
        <div class="card-body p-4">
            <div class="info-box">
                <p><strong>Motivo:</strong> <?= esc($lista['motivo'] ?? '-') ?></p>
                <p><strong>Fecha:</strong> <?= !empty($lista['fecha_actividad']) ? date('d/m/Y', strtotime($lista['fecha_actividad'])) : '-' ?></p>
                <?php if (!empty($lista['convocada_por'])): ?>
                <p><strong>Convocada por:</strong> <?= esc($lista['convocada_por']) ?></p>
                <?php endif; ?>
                <?php if (!empty($lista['lugar'])): ?>
                <p><strong>Lugar:</strong> <?= esc($lista['lugar']) ?></p>
                <?php endif; ?>
            </div>
            ...
            <p class="text-center text-muted mt-3" style="font-size: 11px;">
                Tus datos seran usados unicamente para esta lista de asistencia.
            </p>
        </div>
    </div>
    ...
    document.getElementById('formInscripcion').addEventListener('submit', function(e) {
        ...
        fetch('<?= base_url('lista-asistencia/procesar-inscripcion') ?>', {
            method: 'POST',
            body: fd,
            ...
            if (!data.success) {
                ...
                if (data.duplicado) {
                    Swal.fire({
                        ...
                        text: 'Ya hay un asistente con este numero de documento en esta lista.',
