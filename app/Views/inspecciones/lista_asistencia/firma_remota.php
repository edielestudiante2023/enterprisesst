...
<head>
    ...
    <title>Firma Lista de Asistencia</title>
    ...
    <style>
        ...
        .lista-card { background:white; border-radius:10px; box-shadow:0 1px 6px rgba(0,0,0,0.07); padding:16px; margin-bottom:12px; }
...
<div class="top-bar">
    ...
    <h6><i class="fas fa-clipboard-list me-2"></i>Lista de Asistencia</h6>
    <p><?= esc($cliente['nombre_cliente'] ?? '') ?> &middot; <?= date('d M Y', strtotime($lista['fecha_actividad'])) ?></p>
</div>

<div class="container-fluid px-3 pt-3">
    <div class="aviso-firma mb-3">
        <i class="fas fa-pen-nib me-1"></i>
        Hola <strong><?= esc($asistente['nombre_completo']) ?></strong>, revisa los datos de la convocatoria y firma al final para confirmar tu asistencia.
    </div>

    <div class="lista-card">
        <div class="section-title"><i class="fas fa-clipboard-list"></i> DATOS DE LA CONVOCATORIA</div>
        <div class="row g-3">
            <div class="col-12"><div class="dato-label">Motivo</div><div class="dato-val"><?= esc($lista['motivo']) ?></div></div>
            <div class="col-6"><div class="dato-label">Fecha</div><div class="dato-val"><?= date('d/m/Y', strtotime($lista['fecha_actividad'])) ?></div></div>
            <div class="col-6"><div class="dato-label">Modalidad</div><div class="dato-val"><?= ucfirst($lista['modalidad']) ?></div></div>
            <?php if (!empty($lista['hora_inicio'])): ?>
            <div class="col-6"><div class="dato-label">Hora inicio</div><div class="dato-val"><?= date('g:i A', strtotime($lista['hora_inicio'])) ?></div></div>
            <?php endif; ?>
            <?php if (!empty($lista['hora_fin'])): ?>
            <div class="col-6"><div class="dato-label">Hora fin</div><div class="dato-val"><?= date('g:i A', strtotime($lista['hora_fin'])) ?></div></div>
            <?php endif; ?>
            <?php if (!empty($lista['convocada_por'])): ?>
            <div class="col-12"><div class="dato-label">Convocada por</div><div class="dato-val"><?= esc($lista['convocada_por']) ?></div></div>
            <?php endif; ?>
            <?php if (!empty($lista['lugar'])): ?>
            <div class="col-12"><div class="dato-label">Lugar</div><div class="dato-val"><?= esc($lista['lugar']) ?></div></div>
            <?php endif; ?>
        </div>
    </div>
    ...
    <div class="lista-card">
        <div class="section-title"><i class="fas fa-list-ol"></i> AGENDA</div>
        <p style="margin:0;"><?= nl2br(esc($lista['agenda'])) ?></p>
    </div>
    <?php endif; ?>
...
(function() {
    ...
    btnFirmar.addEventListener('click', function() {
        ...
        Swal.fire({
            ...
            text: 'Tu firma será registrada en la lista. ¿Continuar?',
            icon: 'question',
            ...
            fetch('<?= site_url('lista-asistencia/procesar-firma-remota') ?>', {
                method: 'POST',
                body: fd,
