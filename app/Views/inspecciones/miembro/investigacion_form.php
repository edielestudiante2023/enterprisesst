...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('tipo_evento').addEventListener('change', function() {
        var esAccidente = this.value === 'accidente';
        document.querySelectorAll('.seccion-accidente').forEach(el => el.style.display = esAccidente ? 'block' : 'none');
        document.querySelectorAll('.seccion-incidente').forEach(el => el.style.display = (!esAccidente && this.value === 'incidente') ? 'block' : 'none');
        document.getElementById('titulo-trabajador').textContent = esAccidente ? 'Trabajador Lesionado' : 'Trabajador Involucrado';
    });
    ...
    function collectFormData() {
        const data = {};
        data.tipo_evento = document.querySelector('[name="tipo_evento"]').value;
        data.fecha_evento = document.querySelector('[name="fecha_evento"]').value;
        data.hora_evento = document.querySelector('[name="hora_evento"]').value;
        data.lugar_exacto = document.querySelector('[name="lugar_exacto"]').value;
        data.descripcion_detallada = document.querySelector('[name="descripcion_detallada"]').value;
        data.nombre_trabajador = document.querySelector('[name="nombre_trabajador"]').value;
        data.testigos = [];
        document.querySelectorAll('.testigo-row').forEach(row => {
            const nombre = row.querySelector('[name="testigo_nombre[]"]').value;
            const cargo = row.querySelector('[name="testigo_cargo[]"]').value;
            const decl = row.querySelector('[name="testigo_declaracion[]"]').value;
            if (nombre) data.testigos.push({ nombre, cargo, declaracion: decl });
        });
        data.medidas = [];
        document.querySelectorAll('.medida-row').forEach(row => {
            const desc = row.querySelector('[name="medida_descripcion[]"]').value;
            if (desc) data.medidas.push({ descripcion: desc });
        });
    ...
    function saveToLocal() {
        try {
            ...
            if (data.tipo_evento || data.nombre_trabajador || data.descripcion_detallada) {
    ...
    document.getElementById('investigacionForm').addEventListener('input', function() {
        clearTimeout(window._autoSaveTimeout);
        window._autoSaveTimeout = setTimeout(saveToLocal, 2000);
    });
    document.getElementById('investigacionForm').addEventListener('submit', function() {
