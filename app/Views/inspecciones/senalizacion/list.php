...
<div class="container-fluid px-3">
    ...
    <div class="d-flex justify-content-between align-items-center mt-2 mb-3">
        <h6 class="mb-0">Inspecciones de Senalizacion</h6>
        <a href="<?= site_url('inspecciones/senalizacion/create') ?>" class="btn btn-sm btn-pwa-primary" style="width:auto; padding: 8px 16px;">
        ...
        <table id="tablaInspecciones" class="table table-striped table-bordered table-hover" style="width:100%">
            ...
            <?php foreach (($inspecciones ?? []) as $insp): ?>
                <tr>
                    ...
                    <td><?= !empty($insp['calificacion']) ? number_format($insp['calificacion'], 1) . '%' : '-' ?></td>
                    <td><span class="badge badge-<?= esc($insp['estado']) ?>"><?= $insp['estado'] === 'completo' ? 'Completo' : 'Borrador' ?></span></td>
                    <td class="table-actions">
                        <a href="<?= site_url('inspecciones/senalizacion/edit/' . $insp['id']) ?>" class="btn btn-outline-dark" title="Editar"><i class="fas fa-edit"></i></a>
                        <?php if ($insp['estado'] === 'completo'): ?>
                            <a href="<?= site_url('inspecciones/senalizacion/view/' . $insp['id']) ?>" class="btn btn-outline-dark" title="Ver"><i class="fas fa-eye"></i></a>
                            <a href="<?= site_url('inspecciones/senalizacion/pdf/' . $insp['id']) ?>" class="btn btn-outline-success" target="_blank" title="PDF"><i class="fas fa-file-pdf"></i></a>
                        <?php endif; ?>
                        <a href="<?= site_url('inspecciones/senalizacion/delete/' . $insp['id']) ?>" class="btn btn-outline-danger btn-delete" title="Eliminar"><i class="fas fa-trash"></i></a>
                    </td>
                </tr>
            <?php endforeach; ?>
            </tbody>
