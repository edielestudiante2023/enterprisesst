...
<div class="container-fluid px-3">
    ...
    <div class="mb-4">
        ...
        <a href="/inspecciones/botiquin/pdf/<?= $inspeccion['id'] ?>" class="btn btn-pwa btn-pwa-primary" target="_blank">
            <i class="fas fa-file-pdf"></i> Ver PDF
        </a>
        <?php endif; ?>
    <?php if ($inspeccion['estado'] === 'completo'): ?>
    <a href="/inspecciones/botiquin/regenerar/<?= $inspeccion['id'] ?>" class="btn btn-pwa btn-pwa-outline" onclick="return confirm('Regenerar el PDF con la plantilla actual?')">
        <i class="fas fa-sync-alt me-2"></i>Regenerar PDF
    </a>
    <a href="/inspecciones/botiquin/enviar-email/<?= $inspeccion['id'] ?>" class="btn btn-pwa btn-pwa-outline" onclick="return confirm('¿Enviar el PDF por email al cliente?')">
        <i class="fas fa-envelope me-2"></i>Enviar por Email
    </a>
    <?php endif; ?>

        <!-- Doc 25: edit button always visible regardless of estado -->
        <a href="/inspecciones/botiquin/edit/<?= $inspeccion['id'] ?>" class="btn btn-pwa btn-pwa-outline">
