<div class="container-fluid px-3">
            ...
            <div class="mb-3 pb-3" style="border-bottom:1px solid #eee;">
                ...
                <div class="text-muted" style="font-size:12px; margin-bottom:6px;">
                    <?php if (!empty($h['tipo_epp'])): ?>
                        <i class="fas fa-helmet-safety"></i> <strong><?= esc($h['tipo_epp']) ?></strong>
                    <?php endif; ?>
                    <?php if (!empty($h['trabajador_area'])): ?>
                        &middot; <i class="fas fa-user"></i> <?= esc($h['trabajador_area']) ?>
                    <?php endif; ?>
                </div>
                <?php endif; ?>
    ...
    <div class="mb-4">
        ...
        <a href="<?= site_url('inspecciones/inspeccion-epp/pdf/' . $inspeccion['id']) ?>" class="btn btn-pwa btn-pwa-primary" target="_blank">
            <i class="fas fa-file-pdf"></i> Ver PDF
        </a>
        <?php endif; ?>
        <?php if ($inspeccion['estado'] === 'completo'): ?>
        <a href="<?= site_url('inspecciones/inspeccion-epp/regenerar/' . $inspeccion['id']) ?>" class="btn btn-pwa btn-pwa-outline" onclick="return confirm('¿Regenerar el PDF con la plantilla actual?')">
            <i class="fas fa-sync-alt me-2"></i>Regenerar PDF
        </a>
        <a href="<?= site_url('inspecciones/inspeccion-epp/enviar-email/' . $inspeccion['id']) ?>" class="btn btn-pwa btn-pwa-outline" onclick="return confirm('¿Enviar el PDF por email al cliente, consultor y consultor externo?')">
            <i class="fas fa-envelope me-2"></i>Enviar por Email
        </a>
        <?php endif; ?>

        <?php if ($inspeccion['estado'] !== 'completo'): ?>
        <a href="<?= site_url('inspecciones/inspeccion-epp/edit/' . $inspeccion['id']) ?>" class="btn btn-pwa btn-pwa-outline">
