...
<div class="container-fluid px-3">
    <div class="d-flex justify-content-between align-items-center mt-2 mb-3">
        ...
        <a href="<?= site_url('inspecciones/inspeccion-epp/create') ?>" class="btn btn-sm btn-pwa-primary" style="width:auto; padding: 8px 16px;">
            <i class="fas fa-plus"></i> Nueva
        </a>
    </div>

    <div class="table-responsive">
        <table id="tablaInspeccionesEpp" class="table table-striped table-bordered table-hover" style="width:100%">
            <thead>
                <tr>
                    <th>Cliente</th>
                    <th>Fecha</th>
            ...
            <?php foreach (($inspecciones ?? []) as $insp): ?>
                <tr>
                    ...
                    <td><?= (int)($insp['total_hallazgos'] ?? 0) ?></td>
                    <td><span class="badge badge-<?= esc($insp['estado']) ?>"><?= $insp['estado'] === 'completo' ? 'Completo' : 'Borrador' ?></span></td>
                    <td class="table-actions">
                        <a href="<?= site_url('inspecciones/inspeccion-epp/edit/' . $insp['id']) ?>" class="btn btn-outline-dark" title="Editar"><i class="fas fa-edit"></i></a>
                        <?php if ($insp['estado'] === 'completo'): ?>
                            <a href="<?= site_url('inspecciones/inspeccion-epp/view/' . $insp['id']) ?>" class="btn btn-outline-dark" title="Ver"><i class="fas fa-eye"></i></a>
                            <a href="<?= site_url('inspecciones/inspeccion-epp/pdf/' . $insp['id']) ?>" class="btn btn-outline-success" target="_blank" title="PDF"><i class="fas fa-file-pdf"></i></a>
                        <?php endif; ?>
                        <a href="<?= site_url('inspecciones/inspeccion-epp/delete/' . $insp['id']) ?>" class="btn btn-outline-danger btn-delete" title="Eliminar"><i class="fas fa-trash"></i></a>
                    </td>
                </tr>
            <?php endforeach; ?>
            </tbody>
...
whenDtReady(function($) {
    var table = $('#tablaInspeccionesEpp').DataTable(dtConfigBase({
        order: [[1, 'desc']],
    ...
    $('.btn-delete').on('click', function(e) {
        ...
        const url = this.href;
        Swal.fire({
            title: 'Eliminar inspeccion de EPP?',
