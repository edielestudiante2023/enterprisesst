<?php
/**
 * Vista de Tipo: 2.6.1 Rendicion sobre el desempeno
    ...
    <div class="card-body">
        <div class="row align-items-center">
            ...
            <div class="col-md-4 text-end">
                <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#modalAdjuntar"><i class="bi bi-cloud-upload me-1"></i>Adjuntar Soporte</button>
            </div>
        </div>
    </div>
</div>
...
<div class="alert alert-primary mb-4">
    <div class="d-flex align-items-start">
        ...
        <div>
            <h6 class="mb-1">Rendicion sobre el Desempeno del SG-SST</h6>
            <p class="mb-0 small">Adjunte soportes de rendicion de cuentas: informes de gestion, presentaciones a la alta direccion, actas de rendicion, indicadores de desempeno, resultados de objetivos, etc.</p>
        </div>
    </div>
</div>

<div class="card border-0 shadow-sm mb-4">
    <div class="card-header bg-primary text-white"><h6 class="mb-0"><i class="bi bi-graph-up me-2"></i>Soportes Rendicion de Cuentas</h6></div>
    <div class="card-body">
            ...
            <div class="table-responsive">
                <table class="table table-hover mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td><code><?= esc($s['codigo'] ?? 'SOP-RDD') ?></code></td>
        ...
        <?php else: ?>
            <div class="text-center py-4"><i class="bi bi-graph-up text-muted" style="font-size:2.5rem"></i><p class="text-muted mt-2 mb-0">No hay soportes adjuntados aun.</p></div>
        <?php endif; ?>
    </div>
</div>
...
<div class="modal fade" id="modalAdjuntar" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header bg-primary text-white"><h5 class="modal-title"><i class="bi bi-cloud-upload me-2"></i>Adjuntar Soporte</h5><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div>
            <form action="<?= base_url('documentos-sst/adjuntar-soporte-rendicion') ?>" method="post" enctype="multipart/form-data">
                <div class="modal-body">
                    <input type="hidden" name="id_cliente" value="<?= $cliente['id_cliente'] ?>">
                    <input type="hidden" name="id_carpeta" value="<?= $carpeta['id_carpeta'] ?>">
                    <div class="mb-3"><label class="form-label fw-bold">Tipo de carga</label><div class="btn-group w-100"><input type="radio" class="btn-check" name="tipo_carga" id="tcArch" value="archivo" checked><label class="btn btn-outline-primary" for="tcArch"><i class="bi bi-file-earmark-arrow-up me-1"></i>Archivo</label><input type="radio" class="btn-check" name="tipo_carga" id="tcEnl" value="enlace"><label class="btn btn-outline-primary" for="tcEnl"><i class="bi bi-link-45deg me-1"></i>Enlace</label></div></div>
                    <div class="mb-3" id="cArch"><label class="form-label">Archivo</label><input type="file" class="form-control" name="archivo_soporte" accept=".pdf,.jpg,.jpeg,.png,.xls,.xlsx,.doc,.docx,.ppt,.pptx"></div>
                    ...
                    <div class="mb-3"><label class="form-label">Descripcion</label><input type="text" class="form-control" name="descripcion" required placeholder="Ej: Informe gestion 2026, Presentacion rendicion..."></div>
                ...
                <div class="modal-footer"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button><button type="submit" class="btn btn-primary" id="btnAdj"><i class="bi bi-cloud-upload me-1"></i>Adjuntar</button></div>
            </form>
        </div>
    </div>
</div>
