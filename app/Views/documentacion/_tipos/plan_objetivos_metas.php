<?php
/**
 * Vista de Tipo: 2.2.1 Objetivos definidos, claros, medibles, cuantificables, con metas
 * Carpeta para el Plan de Objetivos y Metas con flujo de FASES
 *
 * FLUJO:
 * 1. Fase 1: Objetivos SG-SST → van al PTA (tipo_servicio = "Objetivos SG-SST")
 * 2. Fase 2: Indicadores de Objetivos → van a tbl_indicadores_sst (categoria = "objetivos_sgsst")
        ...
        <div class="row align-items-center">
            <div class="col-md-8">
                ...
                <p class="text-muted mb-0 small">
                    Adjunte evidencias complementarias: plan de trabajo firmado, cronograma de actividades,
                    actas de seguimiento, evidencias de cumplimiento de metas u otros soportes del plan.
                </p>
            </div>
            <div class="col-md-4 text-end">
                <button type="button" class="btn btn-outline-primary" data-bs-toggle="modal" data-bs-target="#modalAdjuntarSoporteObjetivos">
        ...
        <div class="modal-content">
            <div class="modal-header bg-primary text-white">
                <h5 class="modal-title">
                    <i class="bi bi-cloud-upload me-2"></i>Adjuntar Soporte Plan Objetivos
                </h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>

            <form id="formAdjuntarSoporteObjetivos"
                  action="<?= base_url('documentos-sst/adjuntar-soporte-plan-objetivos') ?>"
                  method="post"
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="btn-group w-100" role="group">
                            <input type="radio" class="btn-check" name="tipo_carga_obj"
                                   id="tipoCargaObjArchivo" value="archivo" checked>
                            <label class="btn btn-outline-primary" for="tipoCargaObjArchivo">
                                <i class="bi bi-file-earmark-arrow-up me-1"></i>Subir Archivo
                            </label>
                            <input type="radio" class="btn-check" name="tipo_carga_obj"
                                   id="tipoCargaObjEnlace" value="enlace">
                            <label class="btn btn-outline-primary" for="tipoCargaObjEnlace">
                    ...
                    <!-- Campo Archivo -->
                    <div class="mb-3" id="campoArchivoObj">
                        <label for="archivo_soporte_obj" class="form-label">Archivo</label>
                        <input type="file" class="form-control" id="archivo_soporte_obj" name="archivo_soporte"
                               accept=".pdf,.jpg,.jpeg,.png,.xls,.xlsx,.doc,.docx">
                        <div class="form-text">PDF, JPG, PNG, Excel, Word. Max: 10MB</div>
                    </div>

                    <!-- Campo Enlace (oculto por defecto) -->
                    <div class="mb-3 d-none" id="campoEnlaceObj">
                        <label for="url_externa_obj" class="form-label">Enlace</label>
                        <input type="url" class="form-control" id="url_externa_obj" name="url_externa"
                               placeholder="https://drive.google.com/...">
                    </div>

                    <!-- Descripción -->
                    <div class="mb-3">
                        <label for="descripcion_obj" class="form-label">Descripcion <span class="text-danger">*</span></label>
                        <input type="text" class="form-control" id="descripcion_obj" name="descripcion" required
                               placeholder="Ej: Plan de trabajo firmado, Acta de seguimiento Q1...">
                    </div>

                    <!-- Año -->
                    <div class="mb-3">
                        <label for="anio_obj" class="form-label">Ano</label>
                        <select class="form-select" id="anio_obj" name="anio">
                    ...
                    <!-- Observaciones -->
                    <div class="mb-3">
                        <label for="observaciones_obj" class="form-label">Observaciones (opcional)</label>
                        <textarea class="form-control" id="observaciones_obj" name="observaciones" rows="2"></textarea>
                    </div>
                </div>

                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
                    <button type="submit" class="btn btn-primary" id="btnAdjuntarObj">
...
<!-- JavaScript para Toggle Archivo/Enlace (IDs únicos para Objetivos) -->
<script>
document.querySelectorAll('input[name="tipo_carga_obj"]').forEach(radio => {
    radio.addEventListener('change', function() {
        const isArchivo = this.value === 'archivo';
        document.getElementById('campoArchivoObj').classList.toggle('d-none', !isArchivo);
        document.getElementById('campoEnlaceObj').classList.toggle('d-none', isArchivo);
        document.getElementById('archivo_soporte_obj').required = isArchivo;
        document.getElementById('url_externa_obj').required = !isArchivo;
        if (!isArchivo) {
            document.getElementById('archivo_soporte_obj').value = '';
        } else {
            document.getElementById('url_externa_obj').value = '';
        }
    });
});

document.getElementById('formAdjuntarSoporteObjetivos')?.addEventListener('submit', function() {
    const btn = document.getElementById('btnAdjuntarObj');
    btn.disabled = true;
