<?php
/**
 * Vista de Tipo: 1.2.3 Responsables del SG-SST con curso virtual de 50 horas
    ...
    <div class="card-body">
        <div class="row align-items-center">
            ...
            <div class="col-md-4 text-end">
                <button type="button" class="btn btn-info text-white" data-bs-toggle="modal" data-bs-target="#modalAdjuntar"><i class="bi bi-cloud-upload me-1"></i>Adjuntar Soporte</button>
            </div>
        </div>
    </div>
</div>
...
<div class="alert alert-info mb-4">
    <div class="d-flex align-items-start">
        ...
        <div>
            <h6 class="mb-1">Responsables del SG-SST con Curso Virtual de 50 Horas</h6>
            <p class="mb-0 small">Adjunte certificados del curso de 50 horas en SST de los responsables del sistema: certificados SENA, ARL u otras entidades autorizadas.</p>
        </div>
    </div>
</div>

<div class="card border-0 shadow-sm mb-4">
    <div class="card-header bg-info text-white"><h6 class="mb-0"><i class="bi bi-mortarboard me-2"></i>Certificados Curso 50 Horas</h6></div>
    <div class="card-body">
            ...
            <div class="table-responsive">
                <table class="table table-hover mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td><code><?= esc($s['codigo'] ?? 'SOP-C50') ?></code></td>
        ...
        <?php else: ?>
            <div class="text-center py-4"><i class="bi bi-mortarboard text-muted" style="font-size:2.5rem"></i><p class="text-muted mt-2 mb-0">No hay certificados adjuntados aun.</p></div>
        <?php endif; ?>
    </div>
</div>
...
<div class="modal fade" id="modalAdjuntar" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header bg-info text-white"><h5 class="modal-title"><i class="bi bi-cloud-upload me-2"></i>Adjuntar Certificado</h5><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div>
            <form action="<?= base_url('documentos-sst/adjuntar-soporte-curso-50h') ?>" method="post" enctype="multipart/form-data">
                <div class="modal-body">
                    <input type="hidden" name="id_cliente" value="<?= $cliente['id_cliente'] ?>">
                    <input type="hidden" name="id_carpeta" value="<?= $carpeta['id_carpeta'] ?>">
                    <div class="mb-3"><label class="form-label fw-bold">Tipo de carga</label><div class="btn-group w-100"><input type="radio" class="btn-check" name="tipo_carga" id="tcArch" value="archivo" checked><label class="btn btn-outline-info" for="tcArch"><i class="bi bi-file-earmark-arrow-up me-1"></i>Archivo</label><input type="radio" class="btn-check" name="tipo_carga" id="tcEnl" value="enlace"><label class="btn btn-outline-info" for="tcEnl"><i class="bi bi-link-45deg me-1"></i>Enlace</label></div></div>
                    <div class="mb-3" id="cArch"><label class="form-label">Archivo</label><input type="file" class="form-control" name="archivo_soporte" accept=".pdf,.jpg,.jpeg,.png"></div>
                    ...
                    <div class="mb-3"><label class="form-label">Descripcion</label><input type="text" class="form-control" name="descripcion" required placeholder="Ej: Certificado 50h Juan Perez, Certificado SENA..."></div>
                ...
                <div class="modal-footer"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button><button type="submit" class="btn btn-info text-white" id="btnAdj"><i class="bi bi-cloud-upload me-1"></i>Adjuntar</button></div>
            </form>
        </div>
    </div>
</div>
