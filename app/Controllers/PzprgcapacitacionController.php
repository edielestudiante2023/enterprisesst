...
class PzprgcapacitacionController extends Controller
{



    public function prgCapacitacion($urlClientId = null)
    {
        ...
        $consultant = $consultantModel->find($client['id_consultor'] ?? 0);
        if (!$consultant) {
        ...
        $policyTypeId = 18; // Supongamos que el ID de la política de alcohol y drogas es 1
        ...
        return view('client/sgsst/1planear/p1_2_1prgcapacitacion', $data);
    }

    public function generatePdf_prgCapacitacion()
    {
        // Instanciar Dompdf
        $dompdf = new Dompdf();
        ...
        $html = view('client/sgsst/1planear/p1_2_1prgcapacitacion', $data);
        ...
        $dompdf->stream('prg_capacitacion.pdf', ['Attachment' => false]);
    }
}
