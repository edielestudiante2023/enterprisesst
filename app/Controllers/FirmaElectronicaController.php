    ...
    private function enviarCorreoFirma(array $solicitud, array $documento): bool
    {
        ...
        try {
            ...
            if ($statusCode < 200 || $statusCode >= 300) {
                log_message('error', "SendGrid firma email RECHAZADO para {$solicitud['firmante_email']} - Status: {$statusCode} - Body: " . substr($body, 0, 500));
            }

            return $statusCode >= 200 && $statusCode < 300;
        } catch (\Exception $e) {
            log_message('error', 'Error enviando email de firma via SendGrid (' . $solicitud['firmante_email'] . '): ' . $e->getMessage());
            return false;
        }
    }
