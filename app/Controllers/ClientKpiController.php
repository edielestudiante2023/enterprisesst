...
class ClientKpiController extends Controller
{
    ...
    public function listClientKpisFull()
    {
        ...
        // Instancia el modelo y utiliza sus métodos para armar la consulta con JOINs
        $clientKpiModel = new \App\Models\ClientKpiModel();

        // Armar la consulta con JOINs usando los métodos del modelo
        $clientKpiModel->select('client_kpi.*, c.nombre_cliente, kp.policy_kpi_definition, o.name_objectives, k.kpi_name, kt.kpi_type, kd.name_kpi_definition, d.data_owner');
        $clientKpiModel->join('client c', 'c.id_cliente = client_kpi.id_cliente');
        $clientKpiModel->join('kpi_policy kp', 'kp.id = client_kpi.id_kpi_policy');
        $clientKpiModel->join('objectives o', 'o.id = client_kpi.id_objectives');
        $clientKpiModel->join('kpis k', 'k.id = client_kpi.id_kpis');
        $clientKpiModel->join('kpi_type kt', 'kt.id = client_kpi.id_kpi_type');
        $clientKpiModel->join('kpi_definition kd', 'kd.id = client_kpi.id_kpi_definition');
        $clientKpiModel->join('data_owner d', 'd.id = client_kpi.id_data_owner', 'left');
        $clientKpiModel->where('client_kpi.id_cliente', $id_cliente);

        // Define cuántos registros mostrar por página
        $perPage = 50;
        $clientKpis = $clientKpiModel->paginate($perPage);
        $pager = $clientKpiModel->pager;
