 ...
 * - Se guardan en tbl_indicadores_sst con categoria = 'mantenimiento_periodico'
 */
class IndicadoresMantenimientoPeriodicoService
{
    protected IndicadorSSTModel $indicadorModel;
    protected const CATEGORIA = 'mantenimiento_periodico';
    protected const NUMERAL = '4.2.5';
    protected const CANTIDAD_GENERAR = 6;
    ...
    public function previewIndicadores(int $idCliente, ?array $contexto = null): array
    {
        ...
        if (empty($apiKey)) {
            log_message('error', 'IndicadoresMantenimiento: OPENAI_API_KEY no configurada');
            return [
                'indicadores' => [],
                'total' => 0,
        ...
        $userPrompt .= "\n\nGenera exactamente " . self::CANTIDAD_GENERAR . " indicadores de Mantenimiento Periodico personalizados para esta empresa.";
        $userPrompt .= "\nDeben ser especificos para su tipo de equipos, instalaciones, nivel de riesgo y actividad economica.";

        $response = $this->llamarOpenAI($systemPrompt, $userPrompt, $apiKey, 0.7);

        if (!$response['success']) {
            log_message('error', 'Error IA Indicadores Mantenimiento: ' . ($response['error'] ?? 'desconocido'));
            return [
                'indicadores' => [],
                'total' => 0,
    ...
    /**
     * System prompt especializado para Mantenimiento Periodico
     */
    protected function construirSystemPrompt(): string
    {
        $cantidad = self::CANTIDAD_GENERAR;
        return "Eres un experto en Seguridad y Salud en el Trabajo (SST) de Colombia, especialista en Mantenimiento Preventivo y Periodico de equipos e instalaciones.
Tu tarea es generar indicadores del Programa de Mantenimiento Periodico personalizados segun el contexto REAL de la empresa.

NORMATIVIDAD APLICABLE:
- Resolucion 0312/2019 - Estandar 4.2.5
...
7. Si hay peligros mecanicos o locativos identificados, los indicadores deben abordarlos
...
11. Temas relevantes: mantenimiento preventivo, fichas tecnicas, hojas de vida de equipos, inspecciones de seguridad a instalaciones, disponibilidad operativa, fallas por mantenimiento inadecuado, inventario de equipos, accidentes por fallas de equipos
12. Responde SOLO en formato JSON valido sin markdown

FORMATO DE RESPUESTA (JSON array):
[{\"nombre\":\"...\",\"tipo\":\"proceso\",\"formula\":\"...\",\"meta\":90,\"unidad\":\"%\",\"periodicidad\":\"trimestral\",\"phva\":\"verificar\",\"numeral\":\"4.2.5\",\"descripcion\":\"...\",\"definicion\":\"...\",\"interpretacion\":\"...\",\"origen_datos\":\"...\",\"cargo_responsable\":\"...\",\"cargos_conocer_resultado\":\"...\"}]";
    }
    ...
    protected function procesarRespuestaIA(string $contenidoIA): array
    {
        ...
        if (!is_array($respuesta)) {
            log_message('error', 'IndicadoresMantenimiento: JSON invalido de IA: ' . substr($contenidoIA, 0, 500));
            return [];
        }
    ...
    protected function llamarOpenAI(string $systemPrompt, string $userPrompt, string $apiKey, float $temperature = 0.7): array
    {
        ...
        log_message('debug', 'IndicadoresMantenimiento llamarOpenAI - modelo: ' . $data['model'] . ', temperature: ' . $temperature);
        ...
        if ($error) {
            log_message('error', 'IndicadoresMantenimiento curl error: ' . $error);
        ...
        if ($httpCode !== 200) {
            ...
            log_message('error', 'IndicadoresMantenimiento OpenAI HTTP ' . $httpCode . ': ' . $errorMsg);
    ...
    /**
     * Genera los indicadores de Mantenimiento Periodico en BD
     */
