...
/**
 * Servicio para generar indicadores del Programa de Inspecciones
...
class IndicadoresInspeccionesService
{
    ...
    protected const CATEGORIA = 'inspecciones';
    protected const NUMERAL = '4.2.4';
    ...
    /**
     * Obtiene el resumen de indicadores de inspecciones para un cliente
     */
    public function getResumenIndicadores(int $idCliente): array
    {
        $indicadores = $this->indicadorModel
            ...
            ->groupStart()
                ...
                ->orLike('nombre_indicador', 'inspeccion', 'both')
                ->orLike('nombre_indicador', 'hallazgo', 'both')
                ->orLike('nombre_indicador', 'condiciones inseguras', 'both')
            ->groupEnd()
            ->findAll();
    ...
    public function previewIndicadores(int $idCliente, ?array $contexto = null): array
    {
        ...
        if (empty($apiKey)) {
            log_message('error', 'IndicadoresInspecciones: OPENAI_API_KEY no configurada');
            return [
                'indicadores' => [],
                'total' => 0,
        ...
        $userPrompt .= "\n\nGenera exactamente " . self::CANTIDAD_GENERAR . " indicadores del Programa de Inspecciones personalizados para esta empresa.";
        $userPrompt .= "\nDeben medir el cumplimiento y eficacia del programa de inspecciones a instalaciones, maquinaria y equipos con participacion del COPASST.";

        $response = $this->llamarOpenAI($systemPrompt, $userPrompt, $apiKey, 0.7);

        if (!$response['success']) {
            log_message('error', 'Error IA Indicadores Inspecciones: ' . ($response['error'] ?? 'desconocido'));
            return [
                'indicadores' => [],
                'total' => 0,
    ...
    /**
     * System prompt especializado para Programa de Inspecciones
     */
    protected function construirSystemPrompt(): string
    {
        $cantidad = self::CANTIDAD_GENERAR;
        return "Eres un experto en Seguridad y Salud en el Trabajo (SST) de Colombia, especialista en inspecciones de seguridad a instalaciones, maquinaria y equipos.
Tu tarea es generar indicadores del Programa de Inspecciones personalizados segun el contexto REAL de la empresa.

NORMATIVIDAD APLICABLE:
- Resolucion 0312/2019 - Estandar 4.2.4: Inspecciones a instalaciones con participacion del COPASST
...
7. Los indicadores deben cubrir: cumplimiento cronograma, participacion COPASST, cobertura areas, cierre hallazgos, eficacia acciones, reduccion condiciones inseguras
...
11. Responde SOLO en formato JSON valido sin markdown

FORMATO DE RESPUESTA (JSON array):
[{\"nombre\":\"...\",\"tipo\":\"proceso\",\"formula\":\"...\",\"meta\":90,\"unidad\":\"%\",\"periodicidad\":\"trimestral\",\"phva\":\"verificar\",\"numeral\":\"4.2.4\",\"descripcion\":\"...\",\"definicion\":\"...\",\"interpretacion\":\"...\",\"origen_datos\":\"...\",\"cargo_responsable\":\"...\",\"cargos_conocer_resultado\":\"...\"}]";
    }
    ...
    protected function procesarRespuestaIA(string $contenidoIA): array
    {
        ...
        if (!is_array($respuesta)) {
            log_message('error', 'IndicadoresInspecciones: JSON invalido de IA: ' . substr($contenidoIA, 0, 500));
            return [];
        }
    ...
    protected function llamarOpenAI(string $systemPrompt, string $userPrompt, string $apiKey, float $temperature = 0.7): array
    {
        ...
        log_message('debug', 'IndicadoresInspecciones llamarOpenAI - modelo: ' . $data['model'] . ', temperature: ' . $temperature);
        ...
        if ($error) {
            log_message('error', 'IndicadoresInspecciones curl error: ' . $error);
        ...
        if ($httpCode !== 200) {
            ...
            log_message('error', 'IndicadoresInspecciones OpenAI HTTP ' . $httpCode . ': ' . $errorMsg);
    ...
    /**
     * Genera los indicadores de inspecciones en BD
     */
    ...
    /**
     * Obtiene los indicadores de inspecciones de un cliente
     */
