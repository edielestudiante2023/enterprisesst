...
<?php
/**
 * Script CLI standalone para crear/verificar tablas de documentos SST
 * NO requiere CodeIgniter - usa PDO puro
 *
 * Uso: php ejecutar_tablas_simple.php
 */
...
CREATE TABLE IF NOT EXISTS `tbl_documentos_sst` (
    ...
    // 2. Crear tabla tbl_doc_versiones_sst
...
CREATE TABLE IF NOT EXISTS `tbl_doc_versiones_sst` (
    ...
    // 3. Agregar foreign key (separado para evitar error si ya existe)
    'fk_version_documento' => "
ALTER TABLE `tbl_doc_versiones_sst`
ADD CONSTRAINT `fk_version_documento_sst`
FOREIGN KEY (`id_documento`) REFERENCES `tbl_documentos_sst` (`id_documento`) ON DELETE CASCADE",
];
...
function ejecutarSQL($pdo, $nombre, $sql) {
    try {
        ...
        // Si el error es "constraint already exists" o "table already exists", no es crítico
        if (strpos($e->getMessage(), 'already exists') !== false ||
            strpos($e->getMessage(), 'Duplicate key name') !== false ||
            strpos($e->getMessage(), 'Duplicate key on write') !== false ||
            strpos($e->getMessage(), 'Duplicate foreign key') !== false ||
            strpos($e->getMessage(), 'errno: 121') !== false ||
            strpos($e->getMessage(), 'errno: 1826') !== false) {
            echo "  ℹ  {$nombre} - Ya existe (OK)\n";
            return true;
        }
