...
### 1. Responsables SST (tbl_cliente_responsables_sst) - 1 registro

```sql
-- Representante Legal OMEGA (id_responsable=3)
DELETE FROM tbl_cliente_responsables_sst WHERE id_cliente = 19;
```

### 2. Contexto SST (tbl_cliente_contexto_sst) - 1 registro
...
### 3. Carpetas documentales (tbl_doc_carpetas) - 65 registros
...
### 4. Estandares del cliente (tbl_cliente_estandares) - 60 registros
...
-- 1. Responsables SST (1 registro)
DELETE FROM tbl_cliente_responsables_sst WHERE id_cliente = 19;

-- 2. Contexto SST (1 registro)
DELETE FROM tbl_cliente_contexto_sst WHERE id_cliente = 19;

-- 3. Carpetas documentales (65 registros, estructura PHVA 2026)
DELETE FROM tbl_doc_carpetas WHERE id_cliente = 19;

-- 4. Estandares del cliente (60 registros)
DELETE FROM tbl_cliente_estandares WHERE id_cliente = 19;
...
SELECT 'tbl_cliente_responsables_sst' AS tabla, COUNT(*) AS registros FROM tbl_cliente_responsables_sst WHERE id_cliente = 19
UNION ALL
